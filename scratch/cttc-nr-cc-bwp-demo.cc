...
main (int argc, char *argv[])
{
  ...
  bool udpFullBuffer = false;     // 주기적인 샘플링 고려 X 따라서, FullBuffer 비활성화
  ...
  bool disableDl = true;    // Downlink 비활성화
  bool disableUl = false;   // Uplink 활성화

  std::string simTag = "default";
  std::string outputDir = "./";

  double simTime = 1; // 시뮬레이션 시간 (단위 : seconds)
  double udpAppStartTime = 0.4; // 앱 시작 시간 (단위 : seconds)
  ...
  // 이동성 설정
  mobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel"); // 노드가 고정된 위치에 있는 모델로 설정, 시뮬레이션 동안 노드가 이동하지 않음
  mobility.SetPositionAllocator (apPositionAlloc);  // 위에서 설정한 위치로 gNB 노드 위치 정의
  mobility.Install (gNbNodes);  // gNB 노드 위치 할당기 설치

  mobility.SetPositionAllocator (staPositionAlloc); // 위에서 설정한 위치로 UE 노드 위치 정의
  mobility.Install (ueNodes); // UE 노드 위치 할당기 설치


  // 5G NR EPC 헬퍼, 빔 포밍 헬퍼 객체 생성
  ...
  // 스펙트럼 구성 설정과 연속 및 비연속 예시
  /*
   * 1) 하나의 작동 대역에 4개의 연속 컴포넌트 캐리어 * (CC)가 배치되며, 이는 ccBwpManager에서 자동으로 생성됩니다 
   * 2) 하나의 작동 대역이 비연속인 경우. CC와 BWP는 수동으로 생성됩니다
  */

  BandwidthPartInfoPtrVector allBwps; // 대역폭 파트 관리 객체
  CcBwpCreator ccBwpCreator;  // CC(Component Carrier)와 BWP 생성을 관리하는 클래스

  OperationBandInfo band;

  // For the case of manual configuration of CCs and BWPs
  // CC 및 BWP를 수동으로 구성하는 경우
  ...
  if (contiguousCc == true) // 연속적인 cc(SCS)
    {
      /*
      ...
      const uint8_t numContiguousCcs = 4; // 전체 대역폭 당 구성된 연속적인 cc의 개수 설정 (4 CCs per Band)

      // CcBwpHelper에 대한 구성 및 생성
  ...
  nrHelper->InitializeOperationBand (&band);  // 작업 대역(가용 대역폭)을 nrhelper에 초기화
  allBwps = CcBwpCreator::GetAllBwps ({band});  // 전체 대역폭을 CcBwp생성자
  ...
  uint32_t bwpIdForLowLat = 0;  // EMBB??
  uint32_t bwpIdForVoice = 1;
  uint32_t bwpIdForVideo = 2;
  uint32_t bwpIdForVideoGaming = 3;

  nrHelper->SetGnbBwpManagerAlgorithmAttribute ("NGBR_LOW_LAT_EMBB", UintegerValue (bwpIdForLowLat)); // attributehelper에서 mMTC 있는지 확인하기
  ...
  if (contiguousCc == true)
    {
      // 넷 디바이스의 속성(enbNetDev.Get (0))과 대역폭 부분(0, (1)을 수동으로 설정합니다.)
    ...
    {
      // 넷디바이스의 속성(enbNetDev.Get (0))과 대역폭 부분(0, (1), ...)을 설정합니다.
